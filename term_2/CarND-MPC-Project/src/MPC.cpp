...
MPC::Solution MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for (int i = 0; i < N - 1; i++)
  {
   	  solution_.x.push_back(solution.x[x_start + 1 + i]);
	  solution_.y.push_back(solution.x[y_start + 1 + i]);
	  solution_.psi.push_back(solution.x[psi_start + 1 + i]);
	  solution_.v.push_back(solution.x[v_start + 1 + i]);
